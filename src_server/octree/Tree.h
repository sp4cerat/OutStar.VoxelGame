...
//			int m = mem[(((i*2/3)+(k*y/vz)*x*z+(j*3/4+vy/4)*x)<<shift)+add]; bonsai
	...
	inline void drawPLY(int t,bool colorizemap=false)
	{
		...
		for(int i=0;i<=c;i++)
		{
			...
			int d = dx.len()*float(vm)*accuracy+1;
			...
			for(int j=0;j<=d;j++)
			{
				...
				if(octree_render)
				{
					...
					*/
					...
					uint spec=float(p.y*4);if(spec>=4)spec=3;

					//set_voxel(  voxel_x,voxel_y,voxel_z, color);
					octree::set_voxel(voxel_x,voxel_y,voxel_z, 
						(p*2.0-vec3f(1,1,1)).encode_normal_uint()+(spec<<24), 
						n.encode_normal_uint());
				}
				else
				{
