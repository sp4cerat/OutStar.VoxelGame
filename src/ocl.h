...
void ocl_init()
{
	...
	if(file_exists("../kernel/kernel.cl"))
	{
		char *ocl_program_source = file_read("../kernel/kernel.cl");
		ocl_kernel_program = ocl_compile(ocl_program_source); 
		file_zwrite("../data/log.txt",ocl_program_source,strlen(ocl_program_source));//char *filename, char* buffer, uint size)
	}
	else
	if(file_exists("../data/log.txt"))
	{
		char *ocl_program_source = file_zread("../data/log.txt");
		ocl_kernel_program = ocl_compile(ocl_program_source); 
	}
	else
	error_stop("Kernel problem!");
}

//#include <map>
//std::map<std::string> _ocl_kernel_list;

cl_kernel ocl_get_kernel(char* name)
{
	cl_kernel k;
	CL_CHECK_ERR(k,clCreateKernel(ocl_kernel_program, name, &_err));
	//_ocl_kernel_list[k]=std::string(name);
	return k;
}

cl_mem ocl_fbo_map(GLuint fbo)
{
	cl_mem mem;
	//CL_CHECK_ERR(mem,clCreateFromGLBuffer(ocl_context, CL_MEM_READ_WRITE, pbo, &_err));
	CL_CHECK_ERR(mem,clCreateFromGLRenderbuffer(ocl_context, CL_MEM_READ_ONLY, fbo, &_err));
	//glFlush();
...
cl_mem ocl_malloc(uint size,void* ptr=0, uint flags=CL_MEM_READ_WRITE)
{
	...
	printf("GPU CL malloc %d kb \t %d MB Total\n",size/(1024),totalsize/(1024*1024));
	return m;
}
...
void ocl_copy_to_device(void* src, cl_mem &dst, size_t size, size_t dstofs=0)
{
	cl_event kernel_completion;
	CL_CHECK(clEnqueueWriteBuffer(ocl_command_queue, dst, CL_TRUE, dstofs, size, src, 0, NULL, &kernel_completion));
	CL_CHECK(clWaitForEvents(1, &kernel_completion));
	CL_CHECK(clReleaseEvent(kernel_completion));
}
...
//#define ocl_param(_ksize, _kptr) { CL_CHECK(clSetKernelArg(*_ocl_current_kernel, _ocl_arg++, _ksize, _kptr)); } 
