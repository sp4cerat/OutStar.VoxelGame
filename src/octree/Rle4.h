...
{	
	...
	void load(char *filename,int palette, int addx,int addy,int addz)
	{
		...
		{
			error_stop("File %s not found",filename);
		}
			...
			for (int x=0;x<map4.sx;x++)
			{
				...
				for (int s=0;s<count;s++,p++)
				{
					...
					for (;y1<y2;y1++,pt++) if (y1<map4.sy)
					{
						...
						if(!palette)
						{
							rgba.x=((255-(tcol&255))&0xff) ;//(int((tcol & 31)<<3)&0xfc);
							rgba.y=rgba.x*0.5;//+(yv+addy);
							rgba.z=rgba.y*0.5;
				...
				//		if(y1<900) rgba.x+=1; else rgba.x+=(((x^yv^slice)>>7)&3);//2;
						...
						octree::set_voxel(x+addx,yv+addy,slice+addz,rgba.to_uint() );//  x+addx+(yv+addy)*256+ (slice+addz)*65536   );//rgba.to_uint() );
					}
				}
			}
		}
